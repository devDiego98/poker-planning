...
export function useBallThrow() {
    ...
    const createBall = useCallback((startX, startY, endX, endY, side) => {
        const id = Date.now();
        return { id, x: startX, y: startY, endX, endY, isFalling: false, vel: { x: 0, y: 0 }, bounceCount: 0, side };
    }, []);

    const throwBallAtElement = useCallback((event) => {
        ...
        const updateBalls = () => {
            setBalls(prevBalls => prevBalls.map(ball => {
                if (ball.isFalling) {
                    ...
                    ball.x = ball.x + (ball.endX - ball.x) * progress;
                    ball.y = ball.y + (ball.endY - ball.y) * progress - 100 * Math.sin(Math.PI * progress);
